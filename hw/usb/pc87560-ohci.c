...
#define NSC_CS_W1C_MASK  (NSC_CS_MEDIA_ERR_OVFL | NSC_CS_UE_PCI | NSC_CS_UE_DESCRIPTOR | \
                           NSC_CS_UE_INTERNAL | NSC_CS_SCHEDULE_ERROR | NSC_CS_PHY_OVERRUN)
                          ...
                          unsigned size)
{
    ...
    switch (addr & ~3) {
    case NSC_CONTROL:
        ohci->nsc_control = (ohci->nsc_control & ~lane) | (wval & NSC_CTL_WRMASK);
